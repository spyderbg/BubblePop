...
/// </summary>
// ReSharper disable once CheckNamespace
public class PoolBoss : MonoSingleton<PoolBoss>
{
	...
	private void EnableNavMeshes()
	{
		if (_navMeshesToEnableByFrameNumber.Count == 0 ||
			!_navMeshesToEnableByFrameNumber.ContainsKey(Time.frameCount))
			return;

		var navMeshesForThisFrame = _navMeshesToEnableByFrameNumber[Time.frameCount];
		if (navMeshesForThisFrame.Count == 0) return;
	...
	public static Transform SpawnOutsidePool(string itemName)
	{
		return Spawn(itemName, Vector3.zero, Quaternion.identity, null);
	}
	...
	/// <param name="itemName">Name of Transform to spawn</param>
	/// <returns>The Transform of the spawned object. It can be null if spawning failed from limits you have set.</returns>
	public static Transform SpawnInPool(string itemName)
	{
		return Spawn(itemName, Vector3.zero, Quaternion.identity, Trans);
	}
	...
	public static Transform Spawn(Transform transToSpawn, Vector3 position, Quaternion rotation, Transform parentTransform)
	{
		...
		if (Instance.autoAddMissingPoolItems) {
			...
			Debug.LogWarning($"The Transform '{itemName}' passed to Spawn method is not configured in Pool Boss.");
			return null;
		}
	...
	public static Transform Spawn(string itemName, Vector3 position, Quaternion rotation, Transform parentTransform)
	{
		...
		if (itemSettings.DespawnedClones.Count == 0) {
			if (!itemSettings.AllowInstantiateMore) {
				if (itemSettings.AllowRecycle) {
					...
					Debug.LogWarning(
						$"The Transform '{itemName}' has no available clones left to Spawn in Pool Boss. Please increase your Preload Qty, turn on Allow Instantiate More or turn on Recycle Oldest (Recycle is only for non-essential things like decals).");
					return null;
				}
			} else {
				...
				if (curCount >= itemSettings.ItemHardLimit) {
					Debug.LogWarning(
						$"The Transform '{itemName}' has reached its item limit in Pool Boss. Please increase your Preload Qty or Item Limit.");
					return null;
				}
				...
				if (Instance.logMessages || itemSettings.LogMessages) {
					Debug.LogWarning(
						$"Pool Boss Instantiated an extra '{itemName}' at {Time.time} because there were none left in the Pool.");
				}
			}
		}
		...
		if (cloneToSpawn == null) {
			Debug.LogWarning(
				$"One or more of the prefab '{itemName}' in Pool Boss has been destroyed. You should never destroy objects in the Pool. Despawn instead. Not spawning anything for this call.");
			return null;
		}
	...
	{
		if (!IsActive(navMeshTransform.gameObject)) return;
	...
	public static List<Transform> CategoryItemsDespawnedList(string category)
	{
		...
		if (Instance == null) return despawnedItems;
	...
	public static List<Transform> CategoryActiveItems(string category)
	{
		...
		if (Instance == null) return activeItems;
	...
	public static void Despawn(Transform transToDespawn, bool keepParent = false) {
		...
		if (!PoolItemsByName.ContainsKey(itemName)) {
			if (Instance.autoAddMissingPoolItems) {
				...
				Debug.LogWarning(
					$"The Transform '{itemName}' passed to Despawn is not in Pool Boss. Not despawning. If you want this to be allowed, check the Auto-Add Missing Items checkbox.");
				return;
			}
		}
	...
	public static void DespawnAllOfPrefab(Transform transToDespawn) {
		...
		if (!PoolItemsByName.ContainsKey(itemName)) {
			Debug.LogWarning(
				$"The Transform '{itemName}' passed to DespawnAllOfPrefab is not in Pool Boss. Not despawning.");
			return;
		}
	...
	public static void DestroyPoolItem(Transform transDeadItem) {
		...
		if (!PoolItemsByName.ContainsKey(itemName)) {
			Debug.LogWarning(
				$"The Transform '{itemName}' passed to DestroyPoolItem is not in Pool Boss. Not despawning.");
			return;
		}
	...
	{
		if (string.IsNullOrEmpty(poolItemName)) return null;
		
		return !PoolItemsByName.ContainsKey(poolItemName)
			? null 
			: PoolItemsByName[poolItemName];
	}
	...
	{
		if (_isReady) return PrefabIsInPool(trans.name);
	...
	public static int PrefabDespawnedCount(Transform transPrefab)
	{
		...
		if (!PoolItemsByName.ContainsKey(itemName)) {
			Debug.LogWarning(
				$"The Transform '{itemName}' passed to PrefabDespawnedCount is not in Pool Boss. Not despawning.");
			return -1;
		}
	...
	public static int PrefabSpawnedCount(Transform transPrefab)
	{
		...
		if (!PoolItemsByName.ContainsKey(itemName)) {
			Debug.LogWarning(
				$"The Transform '{itemName}' passed to PrefabSpawnedCount is not in Pool Boss. Not despawning.");
			return -1;
		}
	...
	public static int PrefabCount {
		get {
			if (_isReady) return PoolItemsByName.Count;
	...
	public static string GetPrefabName(Transform trans)
	{
		...
		if (poolable != null) return poolable.ItemName;
	...
	public static Transform NextPoolItemToSpawn(string itemName)
	{
		...
		if (itemSettings.ItemHardLimit <= totalItems) return null;
	...
	{
		if (!Instance.allowDespawningInactive) return IsActive(go);
		
		var itemName = GetPrefabName(go);
		if (!PoolItemsByName.ContainsKey(itemName)) return false;
